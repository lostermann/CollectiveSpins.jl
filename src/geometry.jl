...
Ring of `N` particles with radius `a` around the origin in the xy-plane.

If distance is set to true, then `a` gives the distance between particles and the radius is determined accordingly.
"""
function ring(a::Real, N::Int; distance = false)
    x = distance ? 0.5*a/sin(pi/N) : a
...
"""
    ...
    Reutrn tangential polarization vectors for a ring of N atoms in the xy-plane created by ring(a, N).
...
"""
    ...
    Calculate the excitation phases created in a geometry by an incidnet plane wave with wave vector k.
    
    Arguments:
    * `k`: Incidentplane wave wave vector.
...
"""
    ...
    Geometry of the LHC-I light harvesting compelx in the xy-plane centered around `z=0` with a mean interatomic distance of `1.0`. Positions `1` to `32` constitute the outer ring, while the inner ring comprises positions `33` to `38`.
"""
function lhc1()
...
"""
    ...
    Normalized polarization vectors in an LHC-I light harvesting complex. Entires `1` to `32` pertain to the outer ring, while entries `33` to `38` give the inner ring's dipole orientations.
